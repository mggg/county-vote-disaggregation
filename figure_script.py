...
for score_type in score_types:
    for score_prefix in score_prefixes:
        ...
        ax = sns.heatmap(table_df, annot=True, cmap = 'YlGn' if score_type == 'r2' else 'YlGn_r', linewidths=1, linecolor='black',fmt=('.0f' if score_type == 'rmse' and score_prefix == 'counts' else '.2f'),annot_kws={"fontsize":14,"weight": "bold"}, vmin = 0 if score_type == 'r2' else None, vmax = 1 if score_type == 'r2' else None)
...
for strategy in [strategy1,strategy2]:
    ...
    scatter_fig(counts_target,counts_pred,scatter_max_dict['counts'],'Actual '+party+' '+mode+' Votes','Estimated '+party+' '+mode+' Votes',party+' '+mode+': '+(strategy.upper().replace('_',' ')), party, figs_dir+'Counts_Scatterplot_'+party+'_'+mode+'_'+strategy+'.png')
    residual_fig(counts_target,counts_pred, resid_min_max_dict['counts'][0],resid_min_max_dict['counts'][1],'Residuals: '+party+' '+mode+' Votes',party+' '+mode+': '+(strategy.upper().replace('_',' ')), party, figs_dir+'Counts_Residuals_'+party+'_'+mode+'_'+strategy+'.png')
    scatter_fig(pct_target,pct_pred, scatter_max_dict['pct'],'Actual '+party+' '+mode+' Percents','Estimated '+party+' '+mode+' Percents',party+' '+mode+': '+(strategy.upper().replace('_',' ')), party, figs_dir+'PCT_Scatterplot_'+party+'_'+mode+'_'+strategy+'.png')
    residual_fig(pct_target,pct_pred, resid_min_max_dict['pct'][0],resid_min_max_dict['pct'][1],'Residuals: '+party+' '+mode+' Percents',party+' '+mode+': '+(strategy.upper().replace('_',' ')), party, figs_dir+'PCT_Residuals_'+party+'_'+mode+'_'+strategy+'.png')
    scatter_fig(partisan_target,partisan_pred, scatter_max_dict['partisan'],'Actual '+party+' '+mode+' Partisan Lean','Estimated '+party+' '+mode+' Partisan Lean',party+' '+mode+': '+(strategy.upper().replace('_',' ')), party, figs_dir+'Partisan_Scatterplot_'+party+'_'+mode+'_'+strategy+'.png')
    residual_fig(partisan_target,partisan_pred, resid_min_max_dict['partisan'][0],resid_min_max_dict['partisan'][1],'Residuals: '+party+' '+mode+' Partisan Lean',party+' '+mode+': '+(strategy.upper().replace('_',' ')), party, figs_dir+'Partisan_Residuals_'+party+'_'+mode+'_'+strategy+'.png')
