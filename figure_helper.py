...
def scatter_fig(target,pred, x_y_max,xlabel,ylabel,title,party,fig_name):
    fig, ax = plt.subplots()
...
def residual_fig(target,pred, x_min,x_max,xlabel,title,party,fig_name):
    fig, ax = plt.subplots()
    resid_list = [pair[0]-pair[1] for pair in zip(pred,target)]
    plt.hist(resid_list,  bins = 200, color = color_dict[party])
    plt.xlabel(xlabel)
